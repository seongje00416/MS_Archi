...
@Injectable()
export class RabbitmqEventListeners {
  private readonly logger = new Logger(RabbitmqEventListeners.name);
  ...
  handleMessageReceivedEvent(payload: any) {
    this.logger.log(`이벤트 감지: 새 RabbitMQ 메시지 수신됨`);
  ...
  @OnEvent('rabbitmq.message.error')
  handleMessageErrorEvent(payload: any) {
    this.logger.error(`이벤트 감지: RabbitMQ 메시지 처리 오류`);
